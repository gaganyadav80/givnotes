...
class UnicornButton extends FloatingActionButton {
  ...
  UnicornButton({this.currentButton, this.labelText, this.labelFontSize = 14.0, this.labelColor, this.labelBackgroundColor, this.labelShadowColor, this.labelHasShadow = true, this.hasLabel = false})
  ...
  Widget returnLabel() {
    return Container(
        ...
        child:
            Text(this.labelText, style: TextStyle(fontSize: this.labelFontSize, fontWeight: FontWeight.bold, color: this.labelColor == null ? Color.fromRGBO(119, 119, 119, 1.0) : this.labelColor)));
  }
...
class _UnicornDialer extends State<UnicornDialer> with TickerProviderStateMixin {
  ...
  @override
  void initState() {
    this._animationController = AnimationController(vsync: this, duration: Duration(milliseconds: widget.animationDuration));

    widget.controller ?? SpeedDialController()
      ..setAnimator(_animationController);

    this._parentController = AnimationController(vsync: this, duration: Duration(milliseconds: 200));
